-- PowerSwitch Database Schema Migration
-- This migration creates all necessary tables for the PowerSwitch application
...
DO $$ BEGIN
  CREATE POLICY "Enable read access for all users" ON public.power_status FOR SELECT USING (true);
EXCEPTION
  WHEN duplicate_object THEN null;
END $$;

DO $$ BEGIN
  CREATE POLICY "Enable insert access for all users" ON public.power_status FOR INSERT WITH CHECK (true);
EXCEPTION
  WHEN duplicate_object THEN null;
END $$;

DO $$ BEGIN
  CREATE POLICY "Enable update access for all users" ON public.power_status FOR UPDATE USING (true);
EXCEPTION
  WHEN duplicate_object THEN null;
END $$;

DO $$ BEGIN
  CREATE POLICY "Enable read access for all users" ON public.power_consumption FOR SELECT USING (true);
EXCEPTION
  WHEN duplicate_object THEN null;
END $$;

DO $$ BEGIN
  CREATE POLICY "Enable insert access for all users" ON public.power_consumption FOR INSERT WITH CHECK (true);
EXCEPTION
  WHEN duplicate_object THEN null;
END $$;

DO $$ BEGIN
  CREATE POLICY "Enable update access for all users" ON public.power_consumption FOR UPDATE USING (true);
EXCEPTION
  WHEN duplicate_object THEN null;
END $$;
...
VALUES ('info', 'PowerSwitch Setup', 'Database initialized successfully', jsonb_build_object('setup_time', NOW()));
